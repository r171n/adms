...
function add() {
	...
	$(".modal-title").text("Tambah Rombel"); // Set Title to Bootstrap modal title
}
...
function edit(id) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			$('[name="wali_user_id"]').val(data.wali_user_id).change();;
			$("#modal").modal("show"); // show bootstrap modal when complete loaded
			$(".modal-title").text("Rombel " + data.kelas_nama); // Set title to Bootstrap modal title
		},
		error: function (jqXHR, textStatus, errorThrown) {
